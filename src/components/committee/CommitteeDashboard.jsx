 ...
 * Shows 6 stat cards, current event hero, recent activity.
 */
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-center">
          <div className="lg:col-span-7 space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              ...
              >
                <Eye className="w-4 h-4 text-taupe" />
      ...
      <div className="bg-white rounded-2xl p-6 border border-taupe/30 shadow-sm">
        <h3 className="text-lg font-bold text-espresso mb-4">Recent Activity</h3>
        <div className="space-y-3">
          {recentActivities.map((activity) => (
            <div key={activity.id} className="flex items-start gap-3 p-3 rounded-lg hover:bg-offWhite/50 transition-colors">
              <div className={`mt-0.5 w-2 h-2 rounded-full shrink-0 ${
                activity.type === "approval" ? "bg-taupe" :
                activity.type === "faculty" ? "bg-espresso" :
                activity.type === "submission" ? "bg-brown" : "bg-taupe/50"
              }`} />
              <div className="flex-1 min-w-0">
                <p className="text-xs text-darkBrown font-medium">{activity.text}</p>
                <p className="text-[10px] text-brown mt-0.5">{activity.time}</p>
              </div>
            </div>
          ))}
        </div>
