...
import { rejectedDeals } from "../../data/mockData";

export default function RejectedDeals() {
  return (
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-2xl border border-taupe/30 shadow-sm">
        <h2 className="text-xl font-bold text-espresso tracking-tight">Rejected Deals</h2>
        <p className="text-xs text-brown mt-1">Sponsorship deals that were rejected or sent back for revision</p>
      </div>
        ...
        {rejectedDeals.map((deal) => (
          <div key={deal.id} className="bg-white rounded-2xl border border-taupe/30 shadow-sm p-5 hover:shadow-md transition-all duration-200">
            <div className="flex items-start justify-between gap-3">
              ...
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-semibold bg-darkBrown/15 text-darkBrown border border-darkBrown/30">
                Rejected
              </span>
            </div>
            ...
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <p className="text-[10px] font-bold text-brown uppercase tracking-wider mb-1.5">Sponsor Provided</p>
              ...
              <div>
                <p className="text-[10px] font-bold text-brown uppercase tracking-wider mb-1.5">College Promised</p>
            ...
            {/* Remarks */}
            <div className="mt-4 bg-offWhite/60 rounded-xl p-4 border border-taupe/20">
              <div className="flex items-center gap-1.5 mb-1.5">
                <AlertTriangle className="w-3.5 h-3.5 text-brown" />
                <p className="text-[10px] font-bold text-brown uppercase tracking-wider">Reason for Rejection</p>
              </div>
              <p className="text-xs text-darkBrown leading-relaxed">{deal.remarks}</p>
            </div>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-brown">
              ...
              <span className="flex items-center gap-1">
                <XCircle className="w-3.5 h-3.5 text-brown" />
                Rejected {deal.rejectedAt} by {deal.rejectedBy}
              </span>
            </div>
          </div>
        ))}
      ...
      {rejectedDeals.length === 0 && (
        <div className="bg-white rounded-2xl p-8 border border-taupe/30 text-center">
          <p className="text-sm text-brown">No rejected deals.</p>
        </div>
      )}
    </div>
  );
